"""
Tennis Composite Win% Model — Backtest (WTA)
Runs composite model against historical WTA data (Jeff Sackmann dataset).
...
# ── Configuration ────────────────────────────────────────────────────────────
YEARS    = list(range(2010, datetime.date.today().year + 1))  # WTA data clean from 2010
...
# Population fallback averages (WTA-specific)
POP_SGW      = 0.58    # WTA service hold rate ~58% (lower than ATP 62%)
POP_RGW      = 0.42
POP_BP       = 0.58
POP_FORM     = 0.50
POP_ACE_RATE = 0.040   # ~4% of serve points are aces (WTA average, lower than ATP)
POP_SS_WON   = 0.47    # ~47% of second serve points won (WTA average)
...
def compute_sgw(first_in_pct, first_won_pct, second_won_pct) -> float:
    ...
    w1 = first_won_pct if not math.isnan(first_won_pct) else 0.62
    w2 = second_won_pct if not math.isnan(second_won_pct) else 0.47
    return f * w1 + (1 - f) * w2
...
def bo3_only(p: float) -> float:
    """WTA is best-of-3 only — no Bo5 adjustment needed."""
    return p
...
def load_year(year: int) -> Optional[pd.DataFrame]:
    """Load WTA match data for a given year from Sackmann."""
...
def run_backtest():
    ...
    for _, row in all_matches.iterrows():
        ...
        rp_w = last_rank_pts.get(wname, 500.0)
        rp_l = last_rank_pts.get(lname, 500.0)
