"""
Tennis Composite Win% Model — Backtest
Runs composite model against historical ATP data (Jeff Sackmann dataset).
...
# ── Configuration ────────────────────────────────────────────────────────────
YEARS    = list(range(2015, datetime.date.today().year + 1))  # auto-includes current season
...
# Bayesian shrinkage prior strength (number of "pseudo-matches" anchoring to population avg)
STAT_PRIOR = 10

# Surface Elo: pseudo-matches anchoring surface Elo toward overall Elo when samples are sparse
...
def bo5_adjust(p: float) -> float:
    """Markov chain Bo5 win probability from single-set win probability."""
    q = 1 - p
...
def _shrink(values: list, pop_avg: float) -> float:
    """Bayesian shrinkage toward population average.
    shrunk = (n * sample_mean + k * pop_avg) / (n + k)
    When n=0 → pop_avg. When n>>k → sample_mean.
    """
    n = len(values)
    if n == 0:
        return pop_avg
...
def _decayed_mean(values: list, pop_avg: float) -> float:
    """Exponentially decayed weighted mean with Bayesian shrinkage.
    Most recent match has weight 1.0; each prior match is multiplied by FORM_DECAY.
    Effective sample size (sum of weights) is used for shrinkage.
    """
    n = len(values)
    if n == 0:
        return pop_avg
    ...
    # Shrink: effective n = w_sum (geometric series, < true n, reflects recency discount)
...
def composite_win_prob(p1_stats, p2_stats, elo_p1, elo_p2, surf_elo_p, rank_p, surface, best_of,
    ...
    """Compute composite win probability for player 1."""
...
def load_year(year: int) -> Optional[pd.DataFrame]:
    """Try Sackmann first, fall back to TML. Both use identical column names."""
    ...
    for label, url in sources:
        try:
            ...
            continue
    print(f"  WARNING: Could not load {year} from any source")
    return None
...
def run_backtest():
    ...
    elo_ratings   = {}
    # Surface-specific Elo: separate ratings per surface, blended toward overall Elo
    # when a player has few surface matches (Bayesian shrinkage via SURFACE_ELO_PRIOR)
    surf_elo: dict   = {"Hard": {}, "Clay": {}, "Grass": {}}
    surf_elo_n: dict = {"Hard": collections.Counter(), "Clay": collections.Counter(), "Grass": collections.Counter()}

    last_rank_pts: dict = {}  # player_name -> most recent ATP ranking points

    player_stats  = PlayerStats()
    h2h: dict     = collections.defaultdict(lambda: [0, 0])  # {frozenset: [p1_wins, total]}

    results    = []
    components = []  # raw component scores per match, used by optimize_weights.py
    ...
    for _, row in all_matches.iterrows():
        ...
        # H2H lookup (before update) — keyed by sorted name pair
        key = tuple(sorted([wname, lname]))
        h2h_wins_w = h2h[key][0] if wname == key[0] else h2h[key][1] - h2h[key][0]
        h2h_total  = h2h[key][1]
        h2h_wins_w = max(0, min(h2h_wins_w, h2h_total))  # clamp

        # Stats and Elo before update
        ...
        # Blended surface Elo: shrink surface rating toward overall Elo by sample count
        # Fall back to overall Elo for rare surfaces (Carpet, etc.)
        ...
        # Rank points ratio (pre-match — use last known rank points)
        ...
        # Update rank points from this match's row data
    ...
    # Elo dicts are already name-keyed — write directly
    ...
    print("-" * 65)
    print(f"{'Accuracy (>50% = correct)':<30} {acc_comp*100:>11.1f}% {acc_elo*100:>9.1f}% {acc_coin*100:>9.1f}%")
    print(f"{'Brier score (lower=better)':<30} {brier_comp:>12.4f} {brier_elo:>10.4f} {brier_coin:>10.4f}")
    print(f"{'Log loss  (lower=better)':<30} {logloss_comp:>12.4f} {logloss_elo:>10.4f} {'—':>10}")
    ...
    # Calibration — symmetric: include both perspectives per match
    # Winner row: predicted prob = comp_win_prob, outcome = 1
    # Loser row:  predicted prob = 1 - comp_win_prob, outcome = 0
    # This gives a full distribution across [0, 1] and a meaningful actual win rate per bucket.
