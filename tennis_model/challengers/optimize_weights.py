"""
Weight optimizer for the Tennis Composite Win% Model — Challenger Circuit.
Loads backtest_components.csv (produced by backtest.py) and finds optimal
per-surface weights using scipy SLSQP.
"""
...
COMPONENT_NAMES = ["return", "elo", "surf_elo", "rank", "ace", "ss_won"]
...
def eval_weights(w, arrays, mask=None):
    wr, we, wse, wrank, wace, wss = w
    rgw1 = 1.0 - arrays["sgw2"]
    rgw2 = 1.0 - arrays["sgw1"]
    elo_p      = arrays["elo_p"]
    surf_elo_p = arrays["surf_elo_p"]
    rank_p     = arrays["rank_p"]
    ace1, ace2 = arrays["ace1"], arrays["ace2"]
    ss1,  ss2  = arrays["ss_won1"], arrays["ss_won2"]
    fat1, fat2 = arrays["fat1"], arrays["fat2"]
    best_of    = arrays["best_of"]
    h2h_wins_w = arrays["h2h_wins_w"]
    h2h_total  = arrays["h2h_total"]

    s1 = (wr*rgw1 + we*elo_p     + wse*surf_elo_p     + wrank*rank_p     + wace*ace1 + wss*ss1) * fat1
    s2 = (wr*rgw2 + we*(1-elo_p) + wse*(1-surf_elo_p) + wrank*(1-rank_p) + wace*ace2 + wss*ss2) * fat2
...
def optimize_for_mask(arrays, mask, label):
    ...
    starts = [
        [1/6]*6,
        [0.25, 0.40, 0.35, 0, 0, 0],
        [0.1, 0.5, 0.3, 0.1, 0, 0],
        [0.2, 0.3, 0.2, 0.2, 0.05, 0.05],
        [0, 0.5, 0.3, 0.2, 0, 0],
        [0.3, 0.4, 0, 0.3, 0, 0],
        [0.2, 0.4, 0, 0.2, 0.1, 0.1],
    ]
    for x0 in starts:
...
def main():
    ...
    print(f"  {len(df):,} Challenger matches loaded.\n")
    ...
    print()
    print("Suggested SURFACE_WEIGHTS for challengers/backtest.py:")
